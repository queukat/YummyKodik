...
- **Stale release cleanup**: opt-in deletion limited to plugin-managed releases absent from a successfully fetched complete Yummy user list; state-tracked streams must still match their hashes. Recognized Jellyfin artwork and structurally valid NFO sidecars may change; manual releases, unknown files, changed streams, and unproven content are retained. Supported older generation states remain eligible for cleanup without becoming eligible for refresh skipping.
