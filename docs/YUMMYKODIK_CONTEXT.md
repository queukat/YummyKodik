...
Version-link changes use the host library manager's batch persistence with `ItemUpdateType.None`, grouped by actual parent. Calling `Video.UpdateToRepositoryAsync` for this purpose also propagates title metadata to local alternate versions in Jellyfin 12 and can cause redundant NFO writes and refresh work. A potentially changed group is reloaded with complete metadata directly through a repository-backed item query before mutation. Removed, moved or incompletely indexed members defer that group; an unchanged group performs no writes. Host persistence and item-update events remain active.

Native `LocalAlternateVersions` within a managed episode group move with the selected primary. Both ends of each native relationship become members of one flat group under that primary; its native children have matching `OwnerId` and `PrimaryVersionId`. Otherwise a later host metadata refresh can restore a former owner and cause repeated repairs. Local paths outside the managed group stay on their existing owner. Explicit alternates exclude children already represented by native links. The no-op check covers native paths and ownership as well as explicit links; a settled group performs no writes. Late host item updates are observed only for a proven, repairable reciprocal native link. The existing merge worker coalesces these requests by episode group using cached host item identities. It reloads the complete affected season and revalidates that group before repair, including explicit-only voices. Proven cycles are repaired during an active refresh batch so recursive host saves cannot accumulate until completion; general event-driven passes remain deferred. Requests arriving during or after a pass remain pending, and normalized link saves do not request another pass.
...
The merge and managed voice catalog must enumerate physical episode versions with `IncludeOwnedItems = true` and `GroupByPresentationUniqueKey = false`. Jellyfin 12 otherwise excludes linked children through its primary-version filter, so an already merged episode can look like a single version and never switch to a newly selected voice.
...
Promoting an alternate clears both `PrimaryVersionId` and the scanner's `OwnerId`. Jellyfin 12 filters owned items from normal episode queries even when they have no primary-version link. A leftover owner also invalidates the merge's no-op check so existing affected groups repair themselves. Native selection updates the widget without a success dialog that could interrupt playback.
...
Incomplete native episode records can join a merge only when the managed filename, season directory, stream episode and complete sibling witnesses prove the same group. A direct native edge and a shared provider key may be witnessed by different complete siblings. Proof is rechecked before persistence. Missing numeric metadata stays untouched and only complete records can become primary. This prevents recursive native ownership from surviving merely because host episode indexing is incomplete.
