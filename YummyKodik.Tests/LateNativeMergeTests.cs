...
internal static class LateNativeMergeTests
{
    ...
    private static async Task RunScopedBatchAsync()
    {
        ...
        preferred.SetPrimaryVersionId(fixture.Primary.Id);
        fixture.Primary.LinkedAlternateVersions = new[]
        {
            new LinkedChild { ItemId = preferred.Id, Type = LinkedChildType.LinkedAlternateVersion }
        };
        ...
        try
        {
            ...
            Require(fixture.Child.OwnerId == preferred.Id && fixture.Primary.OwnerId == preferred.Id &&
                    preferred.OwnerId == Guid.Empty && !preferred.PrimaryVersionId.HasValue,
    ...
    private static async Task RunAlreadyRepairedAsync()
    {
        ...
        fixture.Child.SetPrimaryVersionId(fixture.Primary.Id);
        await fixture.RunWorkerAsync();
            ...
            switch (targetMethod?.Name)
            {
                ...
                case nameof(ILibraryManager.GetItemList):
                    ...
                    if (query.ItemIds.Length == 0)
                    {
                        ...
                        {
                            Require(query.IncludeOwnedItems, "The scoped season query must include native owned versions.");
                            SeasonQueryCount++;
                        }
                    }
